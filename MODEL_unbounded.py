...
def conv_net_block(conv_net, net_info, tensor_list, is_first, is_training, act_o, dilation_rate):
    ...
    #temporaly added if else condition i am certain this has to be revised
    if conv_net['input_index']>0:
        tensor = tensor_list[conv_net['input_index']-1]
    else:
...
def model(net_info, tensor, global_tensor=None, is_training=False, act_o=None, dilation_rate=None, is_first=False):
